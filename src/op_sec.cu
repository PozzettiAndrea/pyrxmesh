...
static std::string write_temp_obj_sec(
    const double* vertices, int nv, const int* faces, int nf)
{
    auto tmp = std::filesystem::temp_directory_path() / "pyrxmesh_sec_in.obj";
    ...
    bool verbose)
{
    ...
    auto tp = clk::now();
    auto in_path = write_temp_obj_sec(vertices, num_vertices, faces, num_faces);
    ...
    if (verbose) {
        fprintf(stderr, "[pyrxmesh] sec: obj_write=%.1fms, mesh_build=%.1fms, gpu=%.1fms, readback=%.1fms\n",
