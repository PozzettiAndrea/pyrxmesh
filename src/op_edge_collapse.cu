// Standalone edge collapse wrapper — collapses edges shorter than threshold.
...
static char* s_ec_argv[] = {(char*)"pyrxmesh", nullptr};
static struct arg {
...
static std::string write_temp_obj_ec(
    const double* vertices, int nv, const int* faces, int nf)
{
    auto tmp = std::filesystem::temp_directory_path() / "pyrxmesh_ecol_in.obj";
...
static MeshResult read_obj_ec(const std::string& path)
{
    ...
    bool verbose)
{
    ...
    auto t0 = clk::now();
    if (verbose)
        fprintf(stderr, "[pyrxmesh] edge_collapse: input %d verts, %d faces, relative_len=%.2f\n",
                num_vertices, num_faces, relative_len);

    auto tp = clk::now();
    auto in_path = write_temp_obj_ec(vertices, num_vertices, faces, num_faces);
    auto out_path = (std::filesystem::temp_directory_path() / "pyrxmesh_ecol_out.obj").string();
    ...
    if (!rx.is_edge_manifold())
        throw std::runtime_error("edge_collapse requires an edge-manifold mesh");
    ...
    for (int iter = 0; iter < iterations; ++iter) {
        collapse_short_edges(rx, coords.get(), edge_status.get(), edge_link.get(),
                             v_boundary.get(), low_edge_len_sq, high_edge_len_sq,
                             timers, d_buffer);
    }
    CUDA_ERROR(cudaDeviceSynchronize());
    ...
    auto result = read_obj_ec(out_path);
    ...
    if (verbose) {
        fprintf(stderr, "[pyrxmesh] edge_collapse: avg_edge=%.4f, threshold=%.4f\n",
                stats.avg_edge_len, low_edge_len);
        fprintf(stderr, "[pyrxmesh] edge_collapse: obj_write=%.1fms, mesh_build=%.1fms, gpu=%.1fms, readback=%.1fms\n",
                t_write, t_build, t_gpu, t_readback);
        fprintf(stderr, "[pyrxmesh] edge_collapse: output %d verts, %d faces, total %.1f ms\n",
