...
bool trace(const std::string& filename_prefix, TraceMesh& traceTrimesh)
{
    ...
    //TRACING
    std::cout << "[STEP] 2.0 Initializing tracer..." << std::endl;
    PTr.InitTracer(Drift,false);

    std::cout << "[STEP] 2.1 Recursive patch tracing..." << std::endl;
    RecursiveProcess<TracerType>(PTr,Drift, add_only_needed,final_removal,true,meta_mesh_collapse,force_split,true,false);
    std::cout << "[STEP] 2.1 Tracing done: " << PTr.ChoosenPaths.size() << " paths" << std::endl;

    std::cout << "[STEP] 2.2 Smoothing patches..." << std::endl;
    PTr.SmoothPatches();
    std::cout << "[STEP] 2.2 Patches smoothed" << std::endl;

    SaveAllData(PTr,filename_prefix,0,false,false);
    std::cout << "[STEP] 2.3 Trace data saved" << std::endl;
    return true;
}
