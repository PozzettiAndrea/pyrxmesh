...
public:
    ...
    // Get the intrinsic mesh connectivity (after Ptolemy flips) with UV.
    // Returns (V_intrinsic, F_intrinsic, uv_intrinsic, FT_intrinsic) where
    // V uses original 3D positions, F encodes the intrinsic connectivity,
    // and uv/FT give the seamless parametrization on the intrinsic mesh.
