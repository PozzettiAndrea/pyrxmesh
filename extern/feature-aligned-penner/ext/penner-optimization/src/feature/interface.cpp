...
AlignedMetricGenerator::get_intrinsic_mesh()
{
    // Export the intrinsic mesh (after Ptolemy flips) with seamless UV.
    //
    // Strategy: compute_layout_VF successfully lays out the type=2 sheet of the
    // double cover. We use the R (reflection) map to mirror that UV onto the
    // type=1 sheet, then extract type=1 faces with original vertex IDs.
    auto& m = opt_dirichlet_metric;

    // 1. Compute scale factors (expanded to n_vertices for double cover)
    VectorX scale_factors = ::Penner::Optimization::best_fit_conformal(embedding_metric, opt_metric_coords);
    std::vector<Scalar> u_ind;
    ::Penner::convert_eigen_to_std_vector(scale_factors, u_ind);
    std::vector<Scalar> u(m.n_vertices(), 0.0);
    for (int vi = 0; vi < m.n_vertices(); vi++) {
        int rep = (!m.v_rep.empty() && vi < (int)m.v_rep.size()) ? m.v_rep[vi] : vi;
        if (rep < (int)u_ind.size()) u[vi] = u_ind[rep];
    }
    spdlog::info("Intrinsic UV: scale factors [{}, {}], u size={}",
                 scale_factors.minCoeff(), scale_factors.maxCoeff(), u.size());

    // Compute cuts on intrinsic mesh
    std::vector<bool> is_cut = ::Penner::Optimization::compute_layout_topology(m, {});

    // 2. Find interior type=1 starting halfedge
    int start_h = -1;
    for (int fi = 0; fi < m.n_faces(); fi++) {
        int he = m.h[fi];
        if (m.type[he] != 1) continue;
        bool interior = true;
        for (int hh : {he, m.n[he], m.n[m.n[he]]}) {
            if (m.type[m.opp[hh]] != 1) { interior = false; break; }
        }
        if (interior) { start_h = he; break; }
    }

    // 3. Layout with actual scale factors + cuts on intrinsic mesh
    auto [u_c, v_c, layout_is_cut] = ::Penner::Optimization::compute_layout_components(m, u, is_cut, start_h);

    // Count NaN in type=1 UV
    int n_nan_t1 = 0, n_good_t1 = 0;
    for (int hi = 0; hi < m.n_halfedges(); hi++) {
        if (m.type[hi] != 1) continue;
        if (std::isnan((double)u_c[hi]) || std::isnan((double)v_c[hi])) n_nan_t1++;
        else n_good_t1++;
    }
    spdlog::info("Type=1 UV: {} good, {} NaN", n_good_t1, n_nan_t1);

    // 4. Extract original-sheet faces (type != 2) with UV
    std::vector<int> orig_faces;
    for (int fi = 0; fi < m.n_faces(); fi++) {
        int he = m.h[fi];
        if (m.type[he] == 2) continue;
        orig_faces.push_back(fi);
    }
    int nF = orig_faces.size();

    // Build F and FT, compact UV
    std::set<int> uv_used;
    Eigen::MatrixXi F_intr(nF, 3);
    Eigen::MatrixXi FT_intr(nF, 3);

    for (int i = 0; i < nF; i++) {
        int fi = orig_faces[i];
        int he = m.h[fi];
        for (int j = 0; j < 3; j++) {
            int v = m.to[m.opp[he]]; // tail
            if (!m.v_rep.empty()) v = m.v_rep[v];
            int orig_v = (v < (int)vtx_reindex.size()) ? V_map[vtx_reindex[v]] : v;
            F_intr(i, j) = orig_v;
            FT_intr(i, j) = he;
            uv_used.insert(he);
            he = m.n[he];
    ...
    std::unordered_map<int, int> uv_remap;
    int uv_count = 0;
    for (int idx : uv_used) uv_remap[idx] = uv_count++;

    Eigen::MatrixXd uv_intr(uv_count, 2);
    for (auto& [old_idx, new_idx] : uv_remap) {
        uv_intr(new_idx, 0) = (double)u_c[old_idx];
        uv_intr(new_idx, 1) = (double)v_c[old_idx];
    }

    for (int i = 0; i < nF; i++)
        for (int j = 0; j < 3; j++)
            FT_intr(i, j) = uv_remap[FT_intr(i, j)];

    int n_nan_out = 0;
    for (int i = 0; i < uv_intr.rows(); i++)
        if (std::isnan(uv_intr(i, 0)) || std::isnan(uv_intr(i, 1))) n_nan_out++;

    spdlog::info("Intrinsic mesh: {} V, {} F, {} UV ({} NaN)",
                 (int)V.rows(), nF, uv_count, n_nan_out);

    return std::make_tuple(V, F_intr, uv_intr, FT_intr);
}
