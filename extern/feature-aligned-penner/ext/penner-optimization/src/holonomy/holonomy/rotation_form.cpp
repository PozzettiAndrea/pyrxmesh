...
// Priority function for halfedges (local to this TU to avoid ODR violation)
namespace {
bool has_priority_local(const Mesh<Scalar>& m, const std::vector<int>& vtx_reindex, int h)
{
    ...
    int h)
{
    ...
    //}
    if (!has_priority_local(m, vtx_reindex, h)) {
