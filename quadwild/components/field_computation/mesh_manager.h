...
public:
    ...
    // Checkpoint directory — set before calling BatchProcess to save intermediates
    static inline std::string checkpoint_dir;
                             ...
                             typename vcg::tri::FieldSmoother<MeshType>::SmoothParam &FieldParam)
    {
        auto save_ckpt = [&](const std::string& name) {
            ...
            std::string path = checkpoint_dir + "/" + name;
            int mask = 0;
            vcg::tri::io::ExporterOBJ<MeshType>::Save(mesh, (path + ".obj").c_str(), mask);
            mesh.SaveSharpFeatures(path + ".sharp");
            std::cout << "[CKPT] Saved " << path << ".obj (" << mesh.vn << "V, " << mesh.fn << "F)" << std::endl;
        };

        auto save_field_ckpt = [&](const std::string& name) {
            if (checkpoint_dir.empty()) return;
            std::string path = checkpoint_dir + "/" + name;
            mesh.SaveField(path + ".rosy");
            // Count and save singularities
            int nsing = 0;
            for (size_t i = 0; i < mesh.vert.size(); i++)
                if (mesh.vert[i].IsS()) nsing++;
            std::cout << "[CKPT] Saved " << path << ".rosy (" << nsing << " singularities)" << std::endl;
        };

        mesh.UpdateDataStructures();
        std::cout << "[STEP] 1.0 Input: " << mesh.vn << "V, " << mesh.fn << "F" << std::endl;
        save_ckpt("step_1_0_input");

        // DETECT SHARP FEATURES (before erode/dilate)
        if (BPar.UpdateSharp) {
            mesh.UpdateDataStructures();
            ...
            for (auto& f : mesh.face)
                for (int j = 0; j < 3; j++)
                    if (f.IsFaceEdgeS(j)) nraw++;
            std::cout << "[STEP] 1.1a Raw feature detection: " << nraw/2 << " feature edges" << std::endl;
            save_ckpt("step_1_1a_features_raw");
            ...
            for (auto& f : mesh.face)
                for (int j = 0; j < 3; j++)
                    if (f.IsFaceEdgeS(j)) neroded++;
            std::cout << "[STEP] 1.1b After erode/dilate(" << BPar.feature_erode_dilate
                      << "): " << neroded/2 << " feature edges" << std::endl;
            save_ckpt("step_1_1b_features_eroded");
        }
        ...
        if (BPar.DoRemesh)
        {
            ...
            std::cout << "[STEP] 1.2 Isotropic remeshing: " << mesh.vn << "V, " << mesh.fn << "F" << std::endl;
            save_ckpt("step_1_2_remeshed");
        }
        mesh.InitFeatureCoordsTable();

        //SOLVE POSSIBLE GEOMETRIC ARTIFACTS AFTER REFINEMENT
        MeshPrepocess<MeshType>::SolveGeometricArtifacts(mesh);
        std::cout << "[STEP] 1.3 SolveGeometricArtifacts: " << mesh.vn << "V, " << mesh.fn << "F" << std::endl;
        save_ckpt("step_1_3_cleaned");

        //REFINE THE MESH IF NEEDED TO BE CONSISTENT WHEN COMPUTING FIELD
        MeshPrepocess<MeshType>::RefineIfNeeded(mesh);
        std::cout << "[STEP] 1.4 RefineIfNeeded: " << mesh.vn << "V, " << mesh.fn << "F" << std::endl;
        save_ckpt("step_1_4_refined");

        MeshFieldSmoother<MeshType>::AutoSetupParam(mesh,FieldParam,BPar.SharpFactor);

        //THEN SMOOTH THE FIELD
        std::cout << "[STEP] 1.5 Computing cross field..." << std::endl;
        ...
        for (size_t i = 0; i < mesh.vert.size(); i++)
            if (mesh.vert[i].IsS()) nsing++;
        std::cout << "[STEP] 1.5 Cross field done (" << nsing << " singularities)" << std::endl;
        save_ckpt("step_1_5_field");
        save_field_ckpt("step_1_5_field");
    }
